...
app.post("/new", function (req, res) {
  ...
  dboperationsUsers
    .insertUsers(username, password, location_id)
    .then((result) => {
...
app.post("/newItemCreated", function (req, res) {
  ...
  dboperationsItems
    .insertItem(i_name, category, price, user_id, reusables)
    .then((result) => {
      res.status(201).json(result);
    });
});
...
app.post("/updateOwnItems", (req, res) => {
  ...
  dboperationsItems
    .updateOwnItems(id, itemName, price, category, reusable)
    .then((result) => {
...
app.post("/updateUserAdmin", (req, res) => {
  ...
  dboperationsUsers
    .updateUserAdmin(user_id, newUsername, newPassword, gold, newRegion)
    .then((result) => {
      res.status(204).json(result);
    });
});
...
app.post("/updateUser", (req, res) => {
  ...
  dboperationsUsers
    .updateUser(user_id, newUsername, newPassword, newRegion)
    .then((result) => {
      res.status(204).json(result);
    });
});
